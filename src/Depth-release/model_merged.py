...
class LDRN(nn.Module):
    ...
    def forward(self, x):
        if self.use_contours:
            ...
            masked3, masked2, masked1, masked0 = torch.zeros(lap4.shape).cuda(), torch.zeros(
                lap3.shape).cuda(), torch.zeros(lap2.shape).cuda(), torch.zeros(lap1.shape).cuda()
            mask3 = (contours3 >= 128).expand((-1, 3, -1, -1))
            mask2 = (contours2 >= 128).expand((-1, 3, -1, -1))
            mask1 = (contours1 >= 128).expand((-1, 3, -1, -1))
            mask0 = (contours0 >= 128).expand((-1, 3, -1, -1))
            masked3[mask3], masked2[mask2], masked1[mask1], masked0[mask0] = lap4[mask3], lap3[mask2], lap2[mask1], \
                                                                             lap1[mask0]
            rgb_list = [None, None, masked3, masked2, masked1, masked0]
        else:
