...
class Lap_decoder_lv5(nn.Module):
    def __init__(self, args, dimList):
        ...
        self.decoder2_reduc1 = myConv(dimList[3] // 4 + dimList[2], dimList[3] // 4 - 5, kSize=1, stride=1, padding=0,
                                      bias=False,
        ...
        self.decoder2_1_reduc2 = myConv(dimList[3] // 8 + dimList[1], dimList[3] // 8 - 5, kSize=1, stride=1, padding=0,
                                        bias=False,
        ...
        self.decoder2_1_1_reduc3 = myConv(dimList[3] // 16 + dimList[0], dimList[3] // 16 - 5, kSize=1, stride=1,
        ...
        self.decoder2_1_1_1_up4 = upConvLayer(dimList[3] // 16, dimList[3] // 16 - 5, 2, norm, act,
                                              (dimList[3] // 16) // 16)
...
class LDRN(nn.Module):
    def __init__(self, args):
        ...
        self.use_contours = args.use_contours
        lv6 = args.lv6
    ...
    def forward(self, x):
        if self.use_contours:
            npt, contours0, contours1, contours2, contours3 = x
            out_featList = self.encoder(npt)
            rgb_down2 = F.interpolate(npt, scale_factor=0.5, mode='bilinear')
            ...
            rgb_up = F.interpolate(rgb_down2, npt.shape[2:], mode='bilinear')
            lap1 = npt - rgb_up
            lap2 = rgb_down2 - rgb_up2
            lap3 = rgb_down4 - rgb_up4
            lap4 = rgb_down8 - rgb_up8
            rgb_list = [None, None, torch.cat([lap4, contours3], dim=1), torch.cat([lap3, contours2], dim=1),
                        torch.cat([lap2, contours1], dim=1), torch.cat([lap1, contours0], dim=1)]
        else:
