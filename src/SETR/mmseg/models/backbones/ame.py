...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qk = nn.Linear(dim * 2, dim * 2, bias=qkv_bias)
    ...
    def forward(self, se, de):
        B, N, C = se.shape
        x = torch.cat([se, de], 2)
        v_se = self.v_se(se).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v_de = self.v_de(de).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        q, k = self.qk(x).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            ...
            if k[:7] == 'blocks.':
                ...
                if ks[2] in ['norm1', 'norm2', 'mlp']:
                    ...
                    if ks[3] == 'proj':
                        ...
                        v_tmp = v[:-1024]
                        if ks[4] == 'weight':
                            v_tmp = torch.cat([v_tmp, v_tmp], 1) / 2
                        ret_dict['.'.join(ks)] = v_tmp
                        ks[3] = 'v_se'
