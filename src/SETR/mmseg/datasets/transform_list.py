...
class RandomColor(object):
    ...
    def __init__(self, multiplier_range=(0.9, 1.1), brightness_mult_range=(0.9, 1.1), dataset='KITTI'):
    ...
    def __call__(self, image):
        if self.dataset == 'KITTI':
            if random.random() < 0.5:
                gamma_mult = np.random.uniform(self.multiplier_range[0],
                                               self.multiplier_range[1])
                imgOut = image ** gamma_mult
                brightness_mult = np.random.uniform(self.brightness_mult_range[0],
                                                    self.brightness_mult_range[1])
                imgOut = imgOut * brightness_mult
                color_mult = np.random.uniform(self.multiplier_range[0],
            ...
            if random.random() < 0.5:
                ...
                result = image
        if random.random() < 0.5:
            ch_pair = self.indices[np.random.randint(1, self.indices_len - 1)]
...
class CenterCrop(object):
    ...
    def __call__(self, img):
        ...
        if img[1].ndim == 3:
            return [im[i:i + h, j:j + w, :] if _is_numpy_image(im) else im for im in img]
        elif img[1].ndim == 2:
            return [im[i:i + h, j:j + w] if _is_numpy_image(im) else im for im in img]
        else:
