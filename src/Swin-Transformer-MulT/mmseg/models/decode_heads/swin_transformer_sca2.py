...
class WindowAttention(nn.Module):
    ...
    def __init__(self, coa, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        if coa:
            self.v = nn.Linear(2 * dim, dim, bias=qkv_bias)
    ...
    def forward(self, x, mask=None):
        ...
        B_, N, C = x.shape
        if self.coa:
            v = self.v(torch.cat([x, co], dim=-1)).reshape(B_, N, self.num_heads, C // self.num_heads).permute(0, 2, 1,
                                                                                                               3)
...
class SwinTransformerSCA2(BaseDecodeHead):
