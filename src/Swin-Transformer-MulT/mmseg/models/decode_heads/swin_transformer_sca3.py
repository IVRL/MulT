...
class WindowAttention(nn.Module):
    ...
    def __init__(self, coa, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        if coa:
            self.qkv = nn.Linear(2 * dim, dim * 3, bias=qkv_bias)
        else:
    ...
    def forward(self, x, mask=None):
        ...
        B_, N, C = x.shape
        if self.coa:
            qkv = self.qkv(torch.cat([x, co], dim=-1)).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2,
                                                                                                                      0,
                                                                                                                      3,
                                                                                                                      1,
                                                                                                                      4)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        if self.coa:
            x_windows = (x_windows, co_windows)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, xHW, coHW, nextHW):
        ...
        assert torch.equal(torch.Tensor([*x.shape]),
        ...
        for blk_enum, blk in enumerate(self.blocks):
            blk.H, blk.W = H, W
            x = (x, co) if blk_enum == 0 else x
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, attn_mask)
            else:
                x = blk(x, attn_mask)
        if self.upsample is not None:
            x_down = self.upsample(x, H, W, nextHW)
            Wh, Ww = nextHW
...
class SwinTransformerSCA3(BaseDecodeHead):
    ...
    def forward(self, inputs):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, H, W, x, Wh, Ww = layer((x, Wh, Ww), inputs[-1 - i],
                                           inputs[-2 - i][1:] if (i < self.num_layers - 1) else (2 * e for e in
                                                                                                 inputs[0][1:]))
